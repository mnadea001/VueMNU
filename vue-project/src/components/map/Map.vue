<template>
  <div id="map">
    ...
    <l-map :zoom="props.zoom" :center="center">
      ...
      <l-marker-cluster-group v-if="!props.event">
        <template v-for="event in props.events" :key="event.id">
          ...
          <l-marker
            ...
            :icon="customMarker"
            :lat-lng="[
              Number(event.place.location.latitude),
              Number(event.place.location.longitude)
            ]"
      ...
      <l-marker
        v-else-if="
          props.event.place.location?.latitude !== null &&
          props.event.place.location?.longitude !== null
        "
        :lat-lng="[
          Number(props.event.place.location.latitude),
          Number(props.event.place.location.longitude)
        ]"
        :icon="customMarker"
      >
        <l-popup :options="popUpOptions">
