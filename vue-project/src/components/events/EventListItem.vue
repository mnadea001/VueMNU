<template>
  ...
  >
    <div class="col-span-1 justify-self-center">
      <img :src="eventImage" alt="event image" class="w-52 h-32 object-cover rounded-lg">
    </div>
    <div class="col-span-2 flex flex-col justify-between">
      <div>
        <h2 class="text-xl font-bold text-gray-800">{{ props.event.name.toUpperCase() }}</h2>
        <h3 class="flex items-center gap-2 mb-2 text-gray-600">
          <IconMarker class="w-5 h-5" />
          {{ eventCity }}, {{ eventState }}
        </h3>
        <p class="text-sm text-gray-700">
          On {{ formatDate(localDate) }} at {{ formatTime(localTime) }}
        </p>
      </div>
    </div>
  </div>
...
const { localTime, localDate } = props.event.dates?.start || {}
